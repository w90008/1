...
typedef struct ftp_env {
  ...
  /* Sonic Loader fork: per-session auth state. When the daemon is
     started anonymous (g_ftp_user empty) `authed` is 1 immediately at
     session init and the auth gate is a no-op. When started with
     -u/-P, `authed` starts at 0 and only flips to 1 after a matching
     USER + PASS pair. */
  int  authed;
...
/* Sonic Loader fork: server-wide auth + transfer-type defaults set
   from main-prospero.c argv (-u USER -P PASS -t auto|binary|ascii).
   Empty user = anonymous (no auth). type='\0' = AUTO mode (let the
   client pick via TYPE I/A); type='I'/'A' forces the per-session
   default and ignores client TYPE overrides. */
extern char g_ftp_user[64];
extern char g_ftp_pass[64];
extern char g_ftp_type;
