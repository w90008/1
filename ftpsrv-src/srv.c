...
/* Sonic Loader fork: server-wide auth + transfer-type defaults
   (extern declarations in cmd.h). main-prospero.c parses argv and
   fills these before ftp_serve() starts accepting connections. */
char g_ftp_user[64] = "";
char g_ftp_pass[64] = "";
char g_ftp_type     = '\0';   /* '\0' = AUTO, 'I' = Binary, 'A' = ASCII */


/* Commands a non-authenticated session is allowed to send. Everything
   else gets a 530 until USER + PASS succeed. */
static int
cmd_is_pre_auth(const char *name) {
  return !strcasecmp(name, "USER") || !strcasecmp(name, "PASS") ||
         !strcasecmp(name, "QUIT") || !strcasecmp(name, "NOOP") ||
         !strcasecmp(name, "FEAT") || !strcasecmp(name, "SYST");
}
...
ftp_execute(ftp_env_t *env, char *line) {
  ...
  /* Sonic Loader fork: gate everything except USER/PASS/QUIT/NOOP/
     FEAT/SYST behind a successful authentication. In anonymous mode
     env->authed is set at session init so this is a no-op. */
  if(!env->authed && !cmd_is_pre_auth(line)) {
    return ftp_active_printf(env, "530 Please login with USER and PASS\r\n");
  }
...
ftp_thread(void *args) {
  ...
  /* Sonic Loader fork: per-session default driven by -t flag.
       g_ftp_type=='I'/'A' -> forced default (TYPE command pins back).
       g_ftp_type=='\0'    -> AUTO, default to Binary (matches upstream
                              behaviour; most clients send TYPE I
                              immediately so this rarely matters). */
  env.type        = (g_ftp_type == 'I' || g_ftp_type == 'A')
                     ? g_ftp_type : 'I';
  env.data_offset = 0;
  env.self2elf    = 1;
  env.authed      = !g_ftp_user[0];   /* anonymous = pre-authed */
  env.pending_user[0] = '\0';
